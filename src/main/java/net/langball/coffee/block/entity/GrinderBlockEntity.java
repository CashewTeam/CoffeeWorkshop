        ...
        if (!level.isClientSide) {
            ...
            if (burnTime == 0 && canSmelt && !fuel.isEmpty()) {
                ...
                if (fuelBurnTime > 0) {
                    ...
                    /* PR-06: set totalCookTime when starting the burn so the
                     * first tick evaluates (1 < 200) and does NOT output. */
                    totalCookTime = getCookTime(input);
                    /* PR-06: capture the crafting remainder BEFORE shrinking
                     * the fuel.  The old code called getCraftingRemainingItem
                     * after shrinking, getting ItemStack.EMPTY. */
            ...
            if (burnTime > 0 && canSmelt) {
                ...
                if (cookTime >= totalCookTime) {
                    totalCookTime = getCookTime(input);
                    cookTime = 0;
                    /* PR-06: cap output at maxStackSize. */
                    if (output.isEmpty()) {
                ...
                /* PR-06: reset cookTime when we can't work, so partial
                 * progress doesn't carry over across different recipes. */
                if (cookTime != 0) {
                    cookTime = 0;
                    dirty = true;
                }
