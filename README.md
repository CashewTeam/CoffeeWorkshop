...
| Metric | Status |
|--------|--------|
| Registered Items | See `docs/content_manifest.json` (`stats.by_type`) — generated per CI run |
| Item Models | 100% (`stats.completeness.model_ok / total_items`) |
| Textures | 100% (`stats.completeness.texture_ok / total_items`) |
| en_us Key Coverage | 100% (`stats.completeness.lang_en_ok / total_items`) |
| zh_cn Key Coverage | 100% (`stats.completeness.lang_zh_ok / total_items`) |
| ja_jp Key Coverage | 100% (`stats.completeness.lang_ja_ok / total_items`) |
| Creative Tab | 100% (`stats.completeness.creative_tab_ok / total_items`) |
| Reachable Sources | See `build/reports/coffeework/recipe-reachability.md` (generated per CI run) |
| GameTests | Assertion counts vary per run; the GameTest server emits pass/fail results to the server log and the `test-reports` artifact |

Refer to the latest CI artifacts (`build/reports/`, `build/test-results/`) for up-to-date
numbers. The legacy matrix is regenerated by `python tools/audit_legacy_runtime_coverage.py`
and committed to `docs/LEGACY_CONTENT_MATRIX.md`.

The GameTest server writes pass/fail per-test to the server log; the per-run
GitHub Actions log for the `Run GameTest server` step is the authoritative
record.  The CI also uploads the entire `build/reports/**` directory as
the `build-reports-${run_number}` artifact.
