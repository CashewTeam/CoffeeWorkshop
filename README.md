...
| GameTests | The Actions log for the `Run GameTest server` step is authoritative; per-test pass/fail lines are also written to `run/logs/latest.log` |

Refer to the latest CI artifact `build-reports-${run_number}` (downloaded from the Actions run) for up-to-date numbers. The legacy matrix is regenerated by `python tools/audit_legacy_runtime_coverage.py` and committed to `docs/LEGACY_CONTENT_MATRIX.md`.
